...
(function (opera) {
    ...
    CmdUtils.openWindow = function openWindow (url,name) {
        CmdUtils.getWindow().setTimeout(function() {
        if (!name) {
    ...
    function ubiq_key_handler (userjs_event) {
        ...
        // ev.ctrlKey is always false on Opera 9.63 on Linux (?)
        var magic_key_pressed = kc==85 && (ev.ctrlKey || ev.metaKey) && ev.shiftKey;

        // If we're in the background (or not created), return immediately
        // Otherwise, activate only on magic key
        if (! ubiq_enabled()) {
            ...
            // Magic key or ESC, hide the Ubiquity window
            if (magic_key_pressed || kc==27) {
                ubiq_toggle_window();
                return;
            }
    ...
    CmdUtils.CreateCommand({
        name: "mini-simulator",
        takes: {"url": noun_arb_text},
        description: "Preview the current URL in the Opera Mini Simulator",
        author: {},
        icon: "http://www.opera.com/favicon.ico",
        homepage: "",
        license: "",
        preview: "Previews the current URL in the Opera Mini simulator",
        execute: CmdUtils.SimpleUrlBasedCommand(
            "http://demo.opera-mini.net/public/index.html?{location}"
        )
    });
    ...
    window.addEventListener('keypress', ubiq_key_handler, false);
